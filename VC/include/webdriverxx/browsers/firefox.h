...
struct FirefoxOptions : JsonObject {
	FirefoxOptions() {};
	explicit FirefoxOptions(const picojson::object& object) : JsonObject(object) {};
...
*/
inline void CustomFromJson(const picojson::value& value, FirefoxOptions& result)
{
	WEBDRIVERXX_CHECK(value.is<picojson::object>(), "FirefoxOptions is not an object");
	result = FirefoxOptions(value.get<picojson::object>());
}
